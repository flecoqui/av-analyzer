...
typedef struct SegmentContext {
	...
	uint32_t rtsp_last_rtcp_ts; // rtps timestamp used if rtsp source
	double rtsp_base_time;
    double rtsp_last_ts;
...
static int is_rtsp_source(const char* source)
{
    // force rtsp source
	if((source[0] == 'r')&&
	(source[1] == 't')&&
...
static double ntp_timestamp(AVFormatContext *pFormatCtx, uint32_t *last_rtcp_ts, double *base_time) {
...
// flecoqui
    av_log(s, AV_LOG_DEBUG, "Input url: %s \n",s->url);
    if(is_rtsp_source(s->url)){
        seg->rtsp_last_ts = ntp_timestamp(s, &seg->rtsp_last_rtcp_ts, &seg->rtsp_base_time);
...
// flecoqui
    av_log(s, AV_LOG_DEBUG, "stream:%d pts:%s pts_time:%s end_pts:%s end_pts_time:%s\n",
           pkt->stream_index,av_ts2str(pkt->pts), av_ts2timestr(pkt->pts, &st->time_base),
		   av_ts2str(end_pts - seg->time_delta), av_ts2timestr(end_pts - seg->time_delta,&AV_TIME_BASE_Q));
    ...
    if (pkt->stream_index == seg->reference_stream_index &&
        ...
        av_log(s, AV_LOG_DEBUG, "End of Segement: stream:%d pts:%s pts_time:%s \n",
        pkt->stream_index,av_ts2str(pkt->pts), av_ts2timestr(pkt->pts, &st->time_base));                            
